...
import ru.LevLezhnin.NauJava.exceptions.user.UserAlreadyBannedException;
import ru.LevLezhnin.NauJava.exceptions.user.UserNotBannedException;
...
public class UserBanServiceImpl implements UserBanService {
    ...
    @Autowired
    public UserBanServiceImpl(UserRepository userRepository, UserBanRepository userBanRepository, Mapper<UserBan, UserBanResponseDto> userBanResponseDtoMapper, RequestContextService requestContextService) {
    ...
    public UserBanResponseDto getUserBanById(Long banId) {
        UserBan userBan = userBanRepository.findById(banId).orElseThrow(() -> new EntityNotFoundException("Блокировка с id: %d не найдена".formatted(banId)));
    ...
    public UserBanResponseDto banUserById(Long userId, String reason) {
        ...
        User user = userRepository.findForUpdateById(userId)
                .orElseThrow(() -> {
                    log.warn("Не найден пользователь, для которого запрошена блокировка. ID администратора: {}, ID пользователя: {}", adminId, userId);
        ...
        log.info("Администратор {} заблокировал пользователя {} по причине: {}. ID блокировки: {}", adminId, userId, reason, userBan.getId());
    ...
    public UserBanResponseDto unbanUserById(Long userId) {
        ...
        if (userOpt.isEmpty()) {
            log.warn("Не найден пользователь, для которого запрошено снятие блокировки. ID администратора: {}, ID пользователя: {}", adminId, userId);
    ...
    public UserBanResponseDto getActiveUserBanByUserId(Long userId) {
        ...
        UserBan activeUserBan = userBanRepository.findActiveUserBan(userId)
                .orElseThrow(() -> {
                    log.warn("Запрошена активная блокировка для незаблокированного пользователя. ID пользователя: {}, ID администратора: {}", userId, requestContextService.getUserId());
    ...
    public List<UserBanResponseDto> getUserBanHistory(Long userId, int page, int pageSize) {
        ...
        if (userRepository.findById(userId).isEmpty()) {
            log.warn("Попытка посмотреть историю блокировок несуществующего пользователя. ID администратора: {}, ID пользователя из запроса: {}", requestContextService.getUserId(), userId);
            throw new EntityNotFoundException("Пользователь с id: %d не найден".formatted(userId));
        }
    ...
    public List<UserBanResponseDto> getIssuedBansByAdmin(Long adminId, int page, int pageSize) {
        ...
        User admin = userRepository.findById(adminId).orElseThrow(() -> {
            log.warn("Попытка посмотреть историю выданных блокировок для несуществующего администратора. ID администратора: {}, ID запрашиваемого администратора: {}", requestContextService.getUserId(), adminId);
            return new EntityNotFoundException("Пользователь с id: %d не найден".formatted(adminId));
        });

        if (!UserRole.ADMIN.equals(admin.getRole())) {
            log.warn("Попытка посмотреть историю выданных блокировок для пользователя (USER). ID администратора: {}, ID пользователя из запроса: {}", requestContextService.getUserId(), adminId);
